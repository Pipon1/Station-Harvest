...
/obj/item/ai_module
	name = "\improper module pour IA"
	icon = 'icons/obj/module.dmi'
	icon_state = "std_mod"
	inhand_icon_state = "electronic"
	lefthand_file = 'icons/mob/inhands/items/devices_lefthand.dmi'
	righthand_file = 'icons/mob/inhands/items/devices_righthand.dmi'
	desc = "Un module pour programmer les lois d'une IA."
	flags_1 = CONDUCT_1
	force = 5
...
/obj/item/ai_module/proc/display_laws()
	...
	if(laws.len)
		assembled_laws += "<B>Lois programmées: [(laws.len > 1) ? "s" : ""]:</B><br>"
		for(var/law in laws)
...
/obj/item/ai_module/proc/install(datum/ai_laws/law_datum, mob/user)
	if(!bypass_law_amt_check && (!laws.len || laws[1] == "")) //So we don't loop trough an empty list and end up with runtimes.
		to_chat(user, span_warning("ERREUR : Pas de loi trouvée"))
		return
	...
	if(law_datum)
		...
		if(tot_laws > CONFIG_GET(number/silicon_max_law_amount) && !bypass_law_amt_check)//allows certain boards to avoid this check, eg: reset
			to_chat(user, span_alert("Pas assez de mémoire allouée au processeur de lois de [law_datum.owner ? law_datum.owner : "le noyau de l'IA"] pour traiter ces changements."))
			message_admins("[ADMIN_LOOKUPFLW(user)] a tenté de téléverser une loi à [law_datum.owner ? ADMIN_LOOKUPFLW(law_datum.owner) : "le noyau de l'IA"] qui aurait dépassé la limite de lois")
			log_silicon("[key_name(user)] a tenté de téléversé une loi à [law_datum.owner ? key_name(law_datum.owner) : "le noyau de l'IA"] qui aurait dépassé la limite de lois")
			overflow = TRUE

	var/law2log = transmitInstructions(law_datum, user, overflow) //Freeforms return something extra we need to log
	if(law_datum.owner)
		to_chat(user, span_notice("Téléversement terminé. Les lois de [law_datum.owner] ont été changées."))
		law_datum.owner.law_change_counter++
	else
		to_chat(user, span_notice("Téléversement terminé."))

	var/time = time2text(world.realtime,"hh:mm:ss")
	var/ainame = law_datum.owner ? law_datum.owner.name : "noyau de l'IA vide"
	var/aikey = law_datum.owner ? law_datum.owner.ckey : "vide"
	...
	GLOB.lawchanges.Add("[time] <B>:</B> [user.name]([user.key]) a utilisé [src.name] sur [ainame]([aikey]).[law2log ? " La loi spécifiée [law2log]" : ""], [length(affected_cyborgs) ? ", ce qui impacte les cyborgs synchronisés [borg_txt]" : ""]")
	log_silicon("LAW: [key_name(user)] a utilisé [src.name] sur [key_name(law_datum.owner)] depuis [AREACOORD(user)].[law2log ? " La loi spécifiée [law2log]" : ""], [length(affected_cyborgs) ? ", ce qui impacte les cyborgs synchronisés [borg_txt]" : ""]")
	message_admins("[ADMIN_LOOKUPFLW(user)] a utilisé [src.name] sur [ADMIN_LOOKUPFLW(law_datum.owner)] depuis [AREACOORD(user)].[law2log ? " La loi spécifiée [law2log]" : ""] , [length(affected_cyborgs) ? ", ce qui impacte les cyborgs synchronisés [borg_flw.Join()]" : ""]")
	if(law_datum.owner)
		deadchat_broadcast("<b> a changé les lois de [span_name("[ainame]")] à l'endroit [get_area_name(user, TRUE)].</b>", span_name("[user]"), follow_target=user, message_type=DEADCHAT_LAWCHANGE)

//The proc that actually changes the silicon's laws.
/obj/item/ai_module/proc/transmitInstructions(datum/ai_laws/law_datum, mob/sender, overflow = FALSE)
	if(law_datum.owner)
		to_chat(law_datum.owner, span_userdanger("[sender] a téléversé un changement dans vos lois en utilisant [name] : vous devez suivre ces changements."))

/obj/item/ai_module/core
	desc = "Un module pour reprogrammer les lois du noyau de l'IA."
...
/obj/item/ai_module/core/round_default_fallback/Initialize(mapload)
	...
	name = "'[default_laws.name]' Module pour loi du noyau de l'IA."
