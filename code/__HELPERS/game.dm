...
/proc/announce_arrival(mob/living/carbon/human/character, rank)
	...
	deadchat_broadcast("<span class='game'> est arrivé sur la station à l'emplacement <span class='name'>[player_area.name]</span>.</span>", "<span class='game'><span class='name'>[character.real_name]</span> ([rank])</span>", follow_target = character, message_type=DEADCHAT_ARRIVALRATTLE)
	if(!character.mind)
		return
...
/proc/send_tip_of_the_round(target, selected_tip)
	...
	to_chat(target, span_purple(examine_block("<span class='oocplain'><b>Conseil du round: </b>[message]</span>")))
